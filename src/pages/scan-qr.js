...
import { Scanner, useDevices } from "@yudiel/react-qr-scanner"; // Use @yudiel/react-qr-scanner
import jsQR from "jsqr"; // For image upload scanning
import Navbar from "@/components/Navbar"; // Verify src/components/Navbar.js exists and has default export
import { Button } from "@/components/ui/button"; // Verify Shadcn/UI button component
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"; // Verify Shadcn/UI card components
import { AlertCircle, Camera, Upload, Scan } from "lucide-react"; // All icons included
import { toast } from "sonner"; // Verify sonner is installed
...
export default function ScanQR() {
  ...
  const devices = useDevices(); // Fetch available cameras using useDevices hook
  ...
  const processScanResult = async (result) => {
    ...
    console.log("Navigating to /registrar-patient-add with query:", parsedData);
    router.push(
      `/registrar-patient-add?${new URLSearchParams(parsedData).toString()}`
    );
  };
            ...
            <div className="relative w-1/2 aspect-square bg-gray-200 dark:bg-gray-800 rounded-md overflow-hidden mx-auto">
              ...
              {isScanning && selectedCamera ? (
                <Scanner
                  ...
                  onScan={handleScan}
                  onError={handleError}
                  formats={["qr_code"]} // Explicitly scan for QR codes
                  paused={!isScanning}
                  styles={{
